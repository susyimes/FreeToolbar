 ...
 */
@CoordinatorLayout.DefaultBehavior(FreeToolbarBehaviorT.class)
public class FreeToolbarTop extends Toolbar {
    ...
    public FreeToolbarTop(Context context) {
        this(context, null);
    }

    public FreeToolbarTop(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, android.support.v7.appcompat.R.attr.toolbarStyle);
    }

    public FreeToolbarTop(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public void hide(boolean animate,int where) {
        mIsHidden = true;
        if (where==0){
        setTranslationY(-this.getHeight(), animate);}else if (where==1){
